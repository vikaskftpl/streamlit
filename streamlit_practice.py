...
@st.cache(allow_output_mutation=True, suppress_st_warning=True)#streamlit 
def download_model():#streamlit 
    model_name = "facebook/mbart-large-50-many-to-many-mmt"
    model = MBartForConditionalGeneration.from_pretrained(model_name)
    tokenizer = MBart50Tokenizer.from_pretrained(model_name)
    return model, tokenizer#streamlit 

st.title('Hindi to English Translator')#streamlit 
text = st.text_area("Enter Text:", value='', height=None, max_chars=None, key=None)
model, tokenizer = download_model()#streamlit 

if st.button('Translate to English'):#streamlit 
    if text == '':#streamlit 
        st.write('Please enter Hindi text for translation') #streamlit 
    else: #streamlit 
        model_name = "facebook/mbart-large-50-many-to-many-mmt"
        ...
        out = tokenizer.batch_decode(generated_tokens, skip_special_tokens=True)#streamlit 
        st.write('', str(out).strip('][\''))#streamlit 
else: pass
